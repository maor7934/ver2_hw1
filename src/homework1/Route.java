...
public class Route {
	...
	// represent a route: a collection of geographical segments that with its
	// features.
	private void checkRep() {
		// TODO : check none of the local variables are null, check that for the segs
		// that p1 = previous.p2 and that length = sum of a segs length (can copy from
		// me)
		// output a message accordingly

		assert (this != null) && (this instanceof Route) && (this.start != null) && (this.end != null)
				&& (this.geoSegments != null) && (this.geoFeatures != null) && (0 <= this.startHeading)
				&& (0 <= this.endHeading) && (this.startHeading < 360) && (this.endHeading < 360)
				&& (this.geoFeatures.size() > 0) && (this.geoSegments.size() > 0) : "Route got illegal parameters";
		...
		for (Iterator<GeoSegment> iterGS = this.geoSegments.iterator(); iterGS.hasNext();) {
			...
			if (lastGS == null) {
				assert this.start.equals(firstGS.getP1()) : "Routes RI violated : first segment p1 != start.";
			} else {
				assert lastGS.getP2().equals(firstGS.getP1()) : "Routes RI violated";
			}
			if (!iterGS.hasNext()) {
				assert firstGS.getP2().equals(this.end) : "Routes RI violated : last segment p2 != end.";
			}
		}
		...
		for (Iterator<GeoFeature> iterGF = this.geoFeatures.iterator(); iterGF.hasNext();) {
			...
			if (lastGF == null) {
				assert this.start.equals(firstGF.getStart()) : "Routes RI violated : first segment p1 != start.";
			} else {
				assert lastGF.getEnd().equals(firstGF.getStart()) : "Routes RI violated" + lastGF.toString() + firstGF.toString();
			}
			if (!iterGF.hasNext()) {
				assert firstGF.getEnd().equals(this.end) : "Routes RI violated : last segment p2 != end.";
			}
		}
		assert (this.length == sumLengthGF) && (this.length == sumLengthGS)
				: "Routes Length isn't equal to GF length or GS length";
	...
	/**
	 * Constructs a new route that is equal to old route, with gs appended to its
	 * end.
	 * 
	 ...
	 **/
	private Route(Route r_old, GeoSegment gs) {
		r_old.checkRep();
		assert (gs != null) && (gs.getP1().equals(r_old.getEnd()))
				: "Route c'tor : gs is null or doesn't continue the route";
		...
		for (Iterator<GeoFeature> iter = r_old.getGeoFeatures(); iter.hasNext();) {
			GeoFeature feature = iter.next();
			if (iter.hasNext()) {
				this.geoFeatures.add(feature);
			} else {
				if (feature.getName().equals(gs.getName())) {
					this.geoFeatures.add(feature.addSegment(gs)); // same feature as last
				} else {
					this.geoFeatures.add(feature); // new feature
					this.geoFeatures.add(new GeoFeature(gs));
				}
			}
		}
	...
	public Route addSegment(GeoSegment gs) {
		this.checkRep();
		assert (gs != null) && (gs.getP1().equals(this.end))
				: "Route addSegment: gs is nukll or doesn't continure the route";
